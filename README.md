...
- 在 Markdown 编辑器或思维导图预览中选中文字并右键，可复制、剪切、粘贴、搜索笔记/标题、创建新笔记并添加链接。按住 Shift 右键可直接使用浏览器原生菜单；“更多浏览器选项”会让下一次右键回到原生菜单。
- 当前笔记的链接入口位于 Markdown 顶部工具栏，面板集中显示反向链接、出站链接、失效目标与索引覆盖率；移动端不会占用编辑器底栏。
