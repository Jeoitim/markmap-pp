...
markmap++ 基于 [Tem-man/markmap-plus](https://github.com/Tem-man/markmap-plus) 开发。它保留了“Markdown → 思维导图”的工作方式，并在此基础上加入完整的本地编辑体验、可回写的节点操作、Mermaid 文档、实验性 WYSIWYG 视觉编辑、IDE 风格仓库文件树、浏览器持久缓存、AI Agent 和手动 GitHub 提交。
...
- Markdown 支持源码模式和实验性的 WYSIWYG 视觉模式；视觉模式实时渲染标题、段落、列表、表格、代码和链接，编辑器会记住浏览器中的模式偏好。
- 编辑器设置可开启浏览器原生拼写检查；在预览设置中开启“点击预览节点定位”后，点击导图节点会跳回对应的 Markdown 内容。
- 桌面端可拖动分割线调整编辑器宽度，也可收起编辑器专注预览。
- 移动端在 Markdown 编辑与思维导图预览之间切换；手机文本选择继续使用系统选中栏，保留全选、查词等系统能力。
- 支持浏览器全屏以及全局浅色、深色主题。

### Mermaid 文档与预览

- 可以从编辑器底栏在 Markdown 与 Mermaid 文档模式之间切换；独立 Mermaid 文档使用源码编辑，避免与 WYSIWYG Markdown 模式混淆。
- Mermaid 文档支持 SVG、PNG、JPEG、PDF 和 Mermaid 源文件导出；网页端 PDF 使用打印对话框，桌面端可以直接保存。
- Markdown 中的 `mermaid` 代码块可在预览设置中开启实验性 SVG 缩略图，支持复制源码、全屏查看和下载 SVG；Markmap 导出仍保留原始代码块。
...
- 预览字号、字体和可变字重均可调整。
- 可关闭点阵背景，获得更简洁的导图画布。
- 开启节点定位后，点击预览中的节点会同步定位编辑器行和对应内容。
...
处于 Mermaid 文档模式时，导出面板会切换为 Mermaid 专用选项：除了图像、SVG 和 PDF，还可以导出 `.mmd` 源文件；PDF 使用当前 Mermaid 图形尺寸，源文件不包含画布样式。
...
- 移动浏览器点击仓库 Markdown 文件后会自动回到编辑器；在文件树上长按仍保留仓库自己的右键菜单。
...
- 在桌面端 Markdown 编辑器或思维导图预览中选中文字并右键，可复制、剪切、粘贴，或从仓库文件树选择笔记；只有主动展开某个文件时才显示其标题，也可搜索文件/标题、创建新笔记并添加链接。按住 Shift 右键可直接使用浏览器原生菜单；“更多浏览器选项”会让下一次右键回到原生菜单。移动端保留系统文本选中栏，自定义浮动操作只补充笔记链接能力，不遮挡全选、查词等系统功能。
