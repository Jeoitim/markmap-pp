...
- **Mermaid 文档**：从编辑器底栏切换 Markdown / Mermaid 文档模式；Mermaid 文档支持独立预览，以及 SVG、PNG、JPEG、PDF 和 `.mmd` 源文件导出。
- **实验性 WYSIWYG 编辑**：Markdown 可在源码模式和视觉模式之间切换，视觉模式实时渲染常见 Markdown 结构，浏览器会记住模式偏好。
- **更顺手的编辑辅助**：编辑器设置增加浏览器拼写检查；开启预览节点定位后，点击思维导图节点即可跳回对应的编辑内容。
- **移动端优化**：手机继续使用系统文本选中栏，自定义浮动操作只补充笔记链接；点击仓库文件会直接回到编辑器，长按文件树仍可使用仓库菜单。
- **更完整的导出**：PDF 使用静态矢量页面，网页端打开打印对话框，桌面端可直接保存；Mermaid 文档拥有独立导出选项。
...
3. 需要保存时，导出 Markdown，或在“仓库”页绑定自己的 GitHub 笔记仓库；需要图形文档时可切换到 Mermaid 模式。
